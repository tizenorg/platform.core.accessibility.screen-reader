...
static void _new_highlighted_obj_changed(AtspiAccessible * new_highlighted_obj, void *user_data)
{
	...
	if (current != new_highlighted_obj) {
		flat_navi_context_current_set(context, g_object_ref(new_highlighted_obj));

		if (atspi_accessible_get_role(new_highlighted_obj, NULL) == ATSPI_ROLE_ENTRY) {
			if (atspi_accessible_get_role(current, NULL) == ATSPI_ROLE_PUSH_BUTTON || atspi_accessible_get_role(current, NULL) == ATSPI_ROLE_EDITBAR) {
				char *text_to_speak = NULL;
