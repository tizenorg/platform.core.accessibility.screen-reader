...
static void _activate_widget(void)
{
	...
	Eina_Bool escape_found = EINA_FALSE;
	gint escape_idx = 0;
	...
	if (action) {
		...
		while (i < number && !activate_found) {
			...
			if (actionName && !strcmp("activate", actionName)) {
				...
				if (!escape_found && actionName && !strcmp("escape", actionName)) {
					escape_found = EINA_TRUE;
					escape_idx = i;
				}
				i++;
			}
		...
		if (activate_found || escape_found) {
			if (!activate_found)
				i = escape_idx;
			...
			ERROR("There is no activate or escape action inside Action interface");
		g_object_unref(action);
	}
