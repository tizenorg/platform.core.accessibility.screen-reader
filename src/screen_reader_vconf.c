...
static void reader_description_cb(keynode_t * node, void *user_data)
{
	DEBUG("START");
	...
	navigator_description_read_enable(descrition);

	DEBUG("END");
}

static void _set_reader_description_vconf_key_changed_callback(Service_Data * service_data)
{
	DEBUG("START");
...
bool vconf_init(Service_Data * service_data)
{
	...
	_set_reader_description_vconf_key_changed_callback(service_data);
