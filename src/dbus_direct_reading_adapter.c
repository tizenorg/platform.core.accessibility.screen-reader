...
_consume_read_command(const Eldbus_Service_Interface *iface EINA_UNUSED, const Eldbus_Message *msg)
{
	...
	if (reply) {
		Eina_Bool interruptible = EINA_TRUE;
		int err_id = 0;
		char* text = "";
		void *rc_id = NULL;
		// read the parameters
		if (!eldbus_message_arguments_get(msg, "sb", &text, &interruptible)) {
			ERROR("eldbus_message_arguments_get() failed to parse arguments of type String and Boolean from message msg=%p", msg);
			err_id = -1;
		} else {
			DEBUG("RECIVED DBUS MESSAGE WITH ARGS: text(%s), interruptable(%d)", text, interruptible);
			rc_id = (void *)tts_speak_customized(text,EINA_TRUE,interruptible, NULL);
		}
		if (!eldbus_message_arguments_append(reply, "sbii", text, interruptible, rc_id, err_id))
...
static const Eldbus_Method _methods[] = {
	{"ReadCommand", ELDBUS_ARGS({"s", "text"},{"b", "interruptable"}), ELDBUS_ARGS({"s", "text"},{"b", "interruptable"},{"i", "id"},{"i", "err_id"}), _consume_read_command},
	{ }
};
...
int dbus_direct_reading_adapter_emit(const Signal signal, const void *data)
{
	...
	if (!strcmp("UnknownSignal", get_signal_name(signal))) {
		ERROR("Trying to emit unknown signal");
		return -1;
	}
	if (!data) {
	...
	if (!eldbus_service_signal_emit(iface, signal, data)) {
