...
_consume_read_command(const Eldbus_Service_Interface *iface EINA_UNUSED, const Eldbus_Message *msg)
{
	...
	void *rc_id = (void *)tts_speak_customized(text,EINA_TRUE,interruptable, NULL);
	eldbus_message_arguments_append(reply, "sbi", text, interruptable,rc_id);
...
static const Eldbus_Method _methods[] = {
	...
	{"ReadCommand", ELDBUS_ARGS({"s", "text"},{"b","interruptable"}), ELDBUS_ARGS({"s", "text"},{"b","interruptable"},{"i","id"}), _consume_read_command},
	{ }
};
...
void dbus_direct_reading_adapter_emit(const Signal signal, const void *data)
{
	if (!iface)
		return;
