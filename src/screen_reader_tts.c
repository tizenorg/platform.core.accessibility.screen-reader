...
static Eina_Bool remove_preceding_read_commands_flag = EINA_FALSE;
...
can_discard(const Read_Command *prev, const Read_Command *next)
{
	...
	return next != NULL ? (prev != NULL ? prev->discardable : EINA_TRUE) : EINA_FALSE;
}
...
Read_Command *get_read_command_from_queue() {
	...
	} while ((eina_list_count(read_command_queue) > 0 && can_discard(command, read_command_queue->data))
			|| command->text == NULL);
	return command;
}
...
void tts_speak_do(Eina_Bool remove_preceding_read_commands)
{
	...
	do {
		...
		if (status == FAILURE_RECOVERABLE) {
			...
			if ((state == TTS_STATE_PLAYING || state == TTS_STATE_PAUSED) && can_discard(last_read_command, command)) {
				...
				if (remove_preceding_read_commands_flag || remove_preceding_read_commands) {
					ret = tts_stop(sd->tts);
...
Read_Command *tts_speak_customized(char *text_to_speak, Eina_Bool remove_preceding_read_commands, Eina_Bool discardable, AtspiAccessible *obj)
{
	...
	tts_speak_do(remove_preceding_read_commands);
	DEBUG("tts_speak_customized: END");
	return rc;
}

Read_Command *tts_speak(char *text_to_speak, Eina_Bool remove_preceding_read_commands)
{
	return tts_speak_customized(text_to_speak, remove_preceding_read_commands, EINA_TRUE, NULL);
}
...
static void __tts_test_utt_completed_cb(tts_h tts, int utt_id, void *user_data)
{
	...
	if (last_utt_id - utt_id > REMOVE_PRECEDING_READ_COMMANDS_LIMIT)
		remove_preceding_read_commands_flag = EINA_TRUE;
	else {
		if (remove_preceding_read_commands_flag)
			remove_preceding_read_commands_flag = EINA_FALSE;
	}
