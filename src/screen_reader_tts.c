...
void overwrite_last_read_command_with(const Read_Command *new_value) {
...
Eina_Bool can_interrupt(const Read_Command *prev, const Read_Command *next) {
	DEBUG("CAN_INTERRUPT: prev (%p), next(%p)", prev, next);
	if (prev != NULL) {
		DEBUG("CAN_INTERRUPT: prev->interruptable(%d), prev->protected(%d)", prev->interruptable, prev->protected);
	}
	return next != NULL ? (prev != NULL ? (prev->interruptable && !prev->protected) : EINA_TRUE) : EINA_FALSE;
}

void tts_speak_do(Eina_Bool flush_switch)
{
	DEBUG("tts_speak_do: BEGIN");
	int ret = 0;
	Service_Data *sd = get_pointer_to_service_data_struct();
	int speak_id;
...
void tts_speak_customized(char *text_to_speak, Eina_Bool flush_switch, Eina_Bool interruptable)
{
	...
	tts_state_e state;
	tts_get_state(sd->tts, &state);
	Eina_Bool protected = EINA_FALSE;
	if ((state == TTS_STATE_PLAYING || state == TTS_STATE_PAUSED) && last_read_command != NULL && !last_read_command->interruptable) {
		protected = EINA_TRUE;
	}
	DEBUG("READ COMMAND PARAMS, TEXT: %s, INTERRUPTABLE: %d, PROTECTED: %d", text_to_speak, interruptable, protected);

	Read_Command *rc = calloc(1,sizeof(Read_Command));
	rc->text = strdup(text_to_speak);
	rc->interruptable = interruptable;
	rc->protected = protected;
...
{
	tts_speak_customized(text_to_speak, flush_switch, EINA_TRUE);
}
...
static void _on_name_cb(void *data, DBusMessage *msg, DBusError *err) {
	DEBUG("On_name_cb");
}